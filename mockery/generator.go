...
func NewGenerator(iface *Interface, pkg string, inPackage bool) *Generator {
	...
	g.addPackageImportWithName("testing", "testing")
	return g
}
...
func (g *Generator) Generate() error {
	...
	newFuncName := "New" + g.mockName()
	if strings.ToLower(g.mockName())[0] == g.mockName()[0] {
		newFuncName = "new" + strings.Title(g.mockName())
	}

	g.printf(`// %s returns a new mock type initialized with test case instance
func %s(t *testing.T) *%s {
	var m %s
	m.Test(t)
	return &m
}
`, newFuncName, newFuncName, g.mockName(), g.mockName())
