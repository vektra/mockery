...
func (s *GeneratorSuite) getGenerator(
	filepath, interfaceName string, inPackage bool, typeAssertion bool,
) *Generator {
	return NewGenerator(
		s.getInterfaceFromFile(filepath, interfaceName), pkg, inPackage, typeAssertion,
	)
}

func (s *GeneratorSuite) checkGeneration(
	filepath, interfaceName string, inPackage bool, typeAssertion bool, expected string,
) *Generator {
	generator := s.getGenerator(filepath, interfaceName, inPackage, typeAssertion)
...
`
	s.checkGeneration(testFile, "Requester", false, false, expected)
}
...
`
	s.checkGeneration(testFile2, "Requester2", false, false, expected)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester3.go"), "Requester3", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester4.go"), "Requester4", false, false,
		expected,
	)
}

func (s *GeneratorSuite) TestGeneratorWithTypeAssertion() {
	expected := `// Requester4 is an autogenerated mock type for the Requester4 type
type Requester4 struct {
	mock.Mock
}

var _ test.Requester4 = (*Requester4)(nil)
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester4.go"), "Requester4", false, true,
		expected,
	)
}
...
`
	s.checkGeneration(
		"requester_unexported.go", "requester_unexported", true, false, expected,
	)
}

func (s *GeneratorSuite) TestGeneratorUnexportedWithTypeAssertion() {
	expected := `// mockRequester_unexported is an autogenerated mock type for the requester_unexported type
type mockRequester_unexported struct {
	mock.Mock
}
...
`
	s.checkGeneration(
		"requester_unexported.go", "requester_unexported", true, true, expected,
	)
}

func (s *GeneratorSuite) TestGeneratorPrologue() {
	generator := s.getGenerator(testFile, "Requester", false, false)
...
func (s *GeneratorSuite) TestGeneratorPrologueWithImports() {
	generator := s.getGenerator("requester_ns.go", "RequesterNS", false, false)
	expected := `package mocks
...
func (s *GeneratorSuite) TestGeneratorPrologueWithMultipleImportsSameName() {
	generator := s.getGenerator("same_name_imports.go", "Example", false, false)
...
func (s *GeneratorSuite) TestVersionOnCorrectLine() {
	gen := s.getGenerator(testFile, "Requester", false, false)
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_ptr.go"), "RequesterPtr", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "requester_ns.go"), "RequesterNS", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsImport", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsNamedImport", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterArgSameAsPkg", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "custom_error.go"), "KeyManager", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		...
		"RequesterReturnElided", false, false, expected,
	)
}
...
func (s *GeneratorSuite) TestGeneratorVariadicArgs() {
	...
	s.checkGeneration(
		...
		"RequesterVariadic", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "func_type.go"), "Fooer", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "async.go"), "AsyncProducer", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "io_import.go"), "MyReader", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "consul.go"), "ConsulLock", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "empty_interface.go"), "Blank", false, false,
		expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "map_func.go"), "MapFunc", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		...
		"UsesOtherPkgIface", false, false, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "mock_method_uses_pkg_iface.go"),
		"UsesOtherPkgIface", true, false, expected,
	)
}

func (s *GeneratorSuite) TestGeneratorForMethodUsingInterfaceInPackageWithTypeAssertion() {
	expected := `// MockUsesOtherPkgIface is an autogenerated mock type for the UsesOtherPkgIface type
type MockUsesOtherPkgIface struct {
	mock.Mock
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "mock_method_uses_pkg_iface.go"),
		"UsesOtherPkgIface", true, true, expected,
	)
}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "same_name_imports.go"), "Example", false, false,
		expected,
	)
}
...
func (s *GeneratorSuite) TestGeneratorWithImportSameAsLocalPackageInpkgNoCycle() {
	...
	pkg := iface.Path
	gen := NewGenerator(iface, pkg, true, false)
	gen.GeneratePrologue(pkg)
...
`
	s.checkGeneration(
		"imports_same_as_package.go", "ImportsSameAsPackage", false, false,
		expected,
	)
}
...
func (s *GeneratorSuite) TestPrologueWithImportFromNestedInterface() {
	generator := s.getGenerator(
		"imports_from_nested_interface.go", "HasConflictingNestedImports", false, false,
	)
	expected := `package mocks
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "struct_value.go"), "A", false, false,
		expected,
	)
}
