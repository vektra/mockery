...
var (
	...
	autoGeneratedRegex = regexp.MustCompile(`^\s*Code generated .* DO NOT EDIT\.(\n)?$`)
)
...
func (r *RootApp) Run() error {
	...
	if len(configuredPackages) == 0 {
		...
		for _, iface := range parser.Interfaces() {
			...
			shouldGenerate, err := r.shouldGenerateInterface(ifaceCtx, iface)
			if err != nil {
				return err
			}
			if !shouldGenerate {
...
func (r *RootApp) shouldGenerateInterface(ctx context.Context, iface *pkg.Interface) (bool, error) {
	var (
		interfaceName = iface.Name
		packageName   = iface.QualifiedName
	)

	shouldGenerate, err := r.Config.ShouldGenerateInterface(ctx, packageName, interfaceName)
	if err != nil {
		return true, err
	}
	...
	pkgConf, err := r.Config.GetPackageConfig(ctx, packageName)
	if err != nil {
		return true, err
	}

	if !pkgConf.IncludeAutoGenerated && isAutoGenerated(iface.File) {
		return false, nil
	}
