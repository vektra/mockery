...
func (s *GeneratorSuite) TestGeneratorGeneric() {
	expected := `// RequesterGeneric is an autogenerated mock type for the RequesterGeneric type
type RequesterGeneric[T interface{}] struct {
	mock.Mock
}

type RequesterGeneric_Get[T interface{}] struct {
	*mock.Call
}

func (_m *RequesterGeneric_Get[T]) Return(_a0 T, _a1 error) *RequesterGeneric_Get[T] {
	return &RequesterGeneric_Get[T]{Call: _m.Call.Return(_a0, _a1)}
}

func (_m *RequesterGeneric[T]) OnGet(path T) *RequesterGeneric_Get[T] {
	c := _m.On("Get", path)
	return &RequesterGeneric_Get[T]{Call: c}
}

func (_m *RequesterGeneric[T]) OnGetMatch(matchers ...interface{}) *RequesterGeneric_Get[T] {
	c := _m.On("Get", matchers...)
	return &RequesterGeneric_Get[T]{Call: c}
}

// Get provides a mock function with given fields: path
func (_m *RequesterGeneric[T]) Get(path T) (T, error) {
	ret := _m.Called(path)
...
type mockConstructorTestingTNewRequesterGeneric interface {
	mock.TestingT
	Cleanup(func())
}

// NewRequesterGeneric creates a new instance of RequesterGeneric. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewRequesterGeneric[T interface{}](t mockConstructorTestingTNewRequesterGeneric) *RequesterGeneric[T] {
	mock := &RequesterGeneric[T]{}
	mock.Mock.Test(t)
...
`
	s.checkGeneration("requester_generic.go", "RequesterGeneric", false, "", expected)
}
