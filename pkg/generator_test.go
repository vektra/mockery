...
func (s *GeneratorSuite) getGenerator(
	...
	return NewGenerator(
		s.ctx, config.Config{InPackage: inPackage}, s.getInterfaceFromFile(filepath, interfaceName), pkg, structName,
	)
}
...
func (s *GeneratorSuite) TestGeneratorWithImportSameAsLocalPackageInpkgNoCycle() {
	...
	gen := NewGenerator(s.ctx, config.Config{InPackage: true}, iface, pkg, "")
	gen.GeneratePrologue(s.ctx, pkg)
