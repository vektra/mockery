...
func NewGenerator(ctx context.Context, config config.Config, iface *Interface, pkg string, structName string) *Generator {
...
func (g *Generator) generateImports(ctx context.Context) {
	...
	for _, name := range g.sortedImportNames() {
		logImport := log.With().Str(logging.KeyLogImport, g.nameToPackagePath[name]).Logger()
		logImport.Debug().Msgf("found import")

		path := g.nameToPackagePath[name]

		if g.Config.InPackage && path == pkgPath {
			logImport.Debug().Msgf("import equals interface package's path, skipping")
			continue
		}
		logImport.Debug().Msgf("writing out import")
		g.printf("import %s \"%s\"\n", name, path)
	}
}
...
// GeneratePrologueNote adds a note before the prologue to the output
// string.
...
func (g *Generator) renderType(ctx context.Context, typ types.Type) string {
	...
	case *types.Named:
		o := t.Obj()
		if o.Pkg() == nil || o.Pkg().Name() == "main" || (g.Config.InPackage && o.Pkg() == g.iface.Pkg) {
			return o.Name()
		}
...
func (g *Generator) Generate(ctx context.Context) error {
	...
	for i := 0; i < g.iface.Type.NumMethods(); i++ {
		...
		called := g.generateCalled(ctx, params, formattedParamNames) // _m.Called invocation string
