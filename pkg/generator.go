...
func (g *Generator) Generate(ctx context.Context) error {
	...
	for _, method := range g.iface.Methods() {
		...
		if len(returns.Types) > 0 {
			retVariable := resolveCollision(setOfParamNames, "args")
			g.printf("\t%s := %s\n", retVariable, called)
			...
			for idx, typ := range returns.Types {
				if typ == "error" {
					ret[idx] = fmt.Sprintf( "%s.Error(%d)", retVariable, idx)
				} else {
					ret[idx] = fmt.Sprintf( "%s.Get(%d).(%s)", retVariable, idx, typ)
				}
			}
