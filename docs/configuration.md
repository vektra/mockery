...
| `require-template-schema-exists`                       | :fontawesome-solid-x:     | `#!yaml true`                         | Specifies whether mockery should assert that the `template-schema` resource exists. If set to `#!yaml false`, mockery will ignore missing schema files and not perform schema validation on `template-data`.                                                |
